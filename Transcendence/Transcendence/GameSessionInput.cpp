		...
		case CTranscendenceWnd::gsInGame:
			...
			//	If we're in a menu, handle it
				...
				if ((iIndex = g_pTrans->m_MenuData.FindItemByKey(sKey)) != -1)
					{
					void* pData = g_pTrans->m_MenuData.GetItemData(iIndex);
					void* pData2 = g_pTrans->m_MenuData.GetItemData2(iIndex);

					switch (m_CurrentMenu)
						{
						case menuComms:
							g_pUniverse->PlaySound(NULL, g_pUniverse->FindSound(UNID_DEFAULT_BUTTON_CLICK));
							g_pTrans->DoCommsMenu((int)(size_t)pData);
							DismissMenu();
						...
						case menuCommsTarget:
							...
							m_pCurrentComms = (CSpaceObject *)pData;
							HideMenu();
							if (m_pCurrentComms)
								ShowMenu(menuComms);
							else
						...
						case menuCommsSquadron:
							g_pUniverse->PlaySound(NULL, g_pUniverse->FindSound(UNID_DEFAULT_BUTTON_CLICK));
							g_pTrans->DoCommsSquadronMenu(g_pTrans->m_MenuData.GetItemLabel(iIndex), (MessageTypes)(DWORD)(size_t)pData, (DWORD)(size_t)pData2);
							DismissMenu();
							m_pCurrentComms = NULL;
							break;
						}
			...
			//	If we're in a picker, handle it
				...
				if (chChar == ']')
					...
					{
					void* pData = g_pTrans->m_MenuData.GetItemData(iEntry);

					switch (m_CurrentMenu)
						{
						case menuUseItem:
							g_pUniverse->PlaySound(NULL, g_pUniverse->FindSound(UNID_DEFAULT_BUTTON_CLICK));
							DoUseMenu((DWORD)(size_t)pData);
							DismissMenu();
							break;
			...
			//	Handle picker
				...
				if (iVirtKey == VK_RETURN)
					...
					switch (m_CurrentMenu)
						...
						case menuUseItem:
							g_pUniverse->PlaySound(NULL, g_pUniverse->FindSound(UNID_DEFAULT_BUTTON_CLICK));
							DoUseMenu((DWORD)(size_t)g_pTrans->m_MenuData.GetItemData(m_PickerDisplay.GetSelection()));
							DismissMenu();
							break;
						...
						case menuEnableDevice:
							g_pUniverse->PlaySound(NULL, g_pUniverse->FindSound(UNID_DEFAULT_BUTTON_CLICK));
							pPlayer->SetUIMessageFollowed(uimsgEnableDeviceHint);
							DoEnableDisableMenu((DWORD)(size_t)g_pTrans->m_MenuData.GetItemData(m_PickerDisplay.GetSelection()));
							DismissMenu();
							break;
						...
						case menuInvoke:
							if (m_PickerDisplay.IsSelectionEnabled())
								{
								GetUniverse().PlaySound(NULL, GetUniverse().FindSound(UNID_DEFAULT_BUTTON_CLICK));
								DoInvokeMenu((DWORD)(size_t)g_pTrans->m_MenuData.GetItemData(m_PickerDisplay.GetSelection()));
								DismissMenu();
								}
							break;
						}
